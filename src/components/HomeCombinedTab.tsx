    ...
    <div id="home-combined-feed" className="px-5 py-4 space-y-5">
      ...
      <div className="flex items-center justify-between pt-1 pb-1">
        <div className="flex items-center gap-1.5">
          <h2 className="text-[22px] font-bold font-sans tracking-tight text-white">Home</h2>
      ...
      {showSimulatorControls && (
        <div className="bg-[#121011] p-3 rounded-xl border border-[#6f6d6c]/10 text-left">
          <span className="text-[11px] text-stone-450 uppercase font-black tracking-widest block mb-2 flex items-center gap-1">
          ...
          <div className="grid grid-cols-3 gap-1.5 font-sans">
            <button
              ...
              className="text-[12px] font-bold bg-[#141414] hover:bg-[#1f1f1f] border border-[#6f6d6c]/10 py-1.5 rounded-lg text-stone-300 transition cursor-pointer"
            >
              Marcus
            </button>
            <button
      ...
      <div className="space-y-6">
        {sortedLogs.map((log) => {
          ...
          return (
            <div
              ...
              className="content-card bg-[#060606] border-[2.3px] border-[#040404] rounded-2xl p-4.5 space-y-4.5 text-left shadow-3d-sm hover:border-[#080808]/25 hover:bg-[#080808] transition-all duration-300 cursor-pointer shadow-[3px_3px_#010101] hover:shadow-[2px_2px_#000000]"
            >
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  <div className={`w-10 h-10 rounded-full overflow-hidden border border-stone-800 flex items-center justify-center ${getAvatarBg(author.username)} text-white font-bold font-sans text-[17px] shadow-3d-sm group-hover/author:border-[#6f6d6c]/60 transition-all`}>
                    {author.avatar_url ? (
                      <img
                  ...
                  <div>
                    <h4 className="text-[15px] font-black text-stone-150 group-hover/author:text-white transition duration-200">
                      {author.username.split(" ")[0].toLowerCase()}
                    </h4>
                    <p className="text-[12px] text-stone-500 font-mono tracking-wide font-bold">
                ...
                <button className="text-stone-605 text-[17px] hover:text-white">•••</button>
              </div>

              {/* Workout Title heading */}
              <div>
                <h3 className="text-[15.5px] font-black text-[#f7f5f4]">{log.title}</h3>
                
                {/* Stats Row */}
                <div className="grid grid-cols-3 gap-2.5 mt-3 border-t border-b border-stone-900/65 py-2">
                  <div>
                    <span className="text-[11px] uppercase font-black tracking-widest text-stone-550 block">Time</span>
                    <span className="font-mono text-[15px] font-black text-stone-105 mt-0.5">
                  ...
                  <div>
                    ...
                    <span className="font-mono text-[15px] font-black text-stone-105 mt-0.5">
                      {Number(log.total_volume).toLocaleString()} <span className="font-sans text-[11px] text-stone-450 uppercase font-black">kg</span>
                    </span>
                  </div>
                  <div>
                    <span className="text-[11px] uppercase font-black tracking-widest text-stone-550 block">Sets</span>
                    <span className="font-mono text-[15px] font-black text-stone-105 mt-0.5 flex items-center gap-1">
              ...
              <div className="space-y-3 pt-1">
                  ...
                  return (
                    <div key={le.id} className="flex items-center gap-3">
                      ...
                      <div className="text-left flex-1 min-w-0">
                        <p className="text-[14px] font-bold text-stone-200 truncate">
                          <span className="font-mono text-stone-400 text-[13px] font-black tracking-tighter mr-1.5">
                            {displaySetsCount} sets
                          </span>
                          {ex?.name}
                        </p>
                        <p className="text-[11.5px] font-mono text-stone-500 font-semibold uppercase tracking-wider mt-0.5">
                ...
                {hasMore && (
                  <button
                    ...
                    className="text-[12.5px] font-black uppercase text-stone-400 hover:text-white transition tracking-widest pl-1 cursor-pointer flex items-center gap-1.5 pt-1"
                  >
        ...
        {sortedLogs.length === 0 && (
          <div className="p-12 text-center text-stone-500 bg-[#1c181a] rounded-2xl border border-dashed border-[#6f6d6c]/25">
            ...
            <p className="text-[15px] font-semibold">Timeline feed empty.</p>
            <p className="text-[13px] mt-1 text-[#6f6d6c] font-mono">Use simulating hotkeys above to instantly stream mock logs!</p>
          </div>
        )}
      </div>
    </div>
