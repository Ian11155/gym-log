    ...
    <div id="profile-tab" className="px-5 py-4 space-y-5">
        ...
        <div className="flex items-center gap-2">
          {!isOwnProfile && (
            <button
              ...
              className="mr-1 py-1 px-2.5 rounded-lg bg-[#242022] border border-[#6f6d6c]/25 hover:bg-[#2f2a2c] text-stone-300 hover:text-white transition duration-200 cursor-pointer text-[13px] font-black uppercase tracking-wider flex items-center font-sans"
            >
              ← Back
            </button>
          )}
          <h2 className="text-[22px] font-bold font-sans tracking-tight text-white">
            {isOwnProfile ? "Athlete Profile" : `${currentUser.username.split(" ")[0]}'s Stats`}
          </h2>
        </div>
        <span className="text-[12px] bg-stone-900 border border-[#6f6d6c]/20 px-2.5 py-1 text-stone-400 rounded-lg font-mono">
          {currentUser.username.toLowerCase()}
        </span>
      </div>

      {/* Profile Info Header Bar (Exact match of the requested UI) */}
      <div className="flex items-center justify-between bg-[#141414] p-4 rounded-xl border-2 border-black shadow-[6px_6px_0px_0px_#000000] animate-fade-in font-sans">
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <h2 className="text-[17px] font-black text-[#f7f5f4]">{currentUser.username}</h2>
            <div className="text-[11px] text-[#6f6d6c] font-bold uppercase tracking-wider font-mono mt-0.5">SQUAD MEMBER</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 bg-[#141414] border-2 border-black px-3 py-1.5 rounded-lg shadow-[3px_3px_0px_0px_#000000]">
          ...
          <div className="text-left">
            <div className="text-[10px] uppercase font-black text-stone-400 tracking-widest font-sans">Streak</div>
            <div className="font-mono text-[15px] font-semibold text-stone-200">{currentUser.streak} Weeks</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 animate-fade-in">
        <div className="bg-[#141414] p-3 rounded-xl border-2 border-black flex items-center gap-2.5 shadow-[4px_4px_0px_0px_#000000]">
          ...
          <div className="text-left">
            <div className="text-[11px] text-stone-400 font-bold uppercase tracking-wider font-sans">Workouts</div>
            <div className="font-mono text-[19px] font-black text-white">{totalWorkouts}</div>
          </div>
        </div>
        ...
        <div className="bg-[#141414] p-3 rounded-xl border-2 border-black flex items-center gap-2.5 shadow-[4px_4px_0px_0px_#000000]">
          ...
          <div className="text-left flex-1 min-w-0">
            <div className="text-[11px] text-stone-400 font-bold uppercase tracking-wider font-sans">Total Volume</div>
            <div className="font-mono text-[19px] font-black text-white truncate max-w-[100px]">
              {totalVolume.toLocaleString()} <span className="text-[11px] text-stone-400 font-normal">kg</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 animate-fade-in">
        <div className="bg-[#141414] p-3 rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_#000000]">
          <div className="text-[11px] text-stone-400 font-bold uppercase tracking-wider font-sans">Completed Sets</div>
          <div className="font-mono text-[19px] font-black text-white mt-1">{profileStats.totalCompletedSets}</div>
        </div>
        <div className="bg-[#141414] p-3 rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_#000000]">
          <div className="text-[11px] text-stone-400 font-bold uppercase tracking-wider font-sans">Training Time</div>
          <div className="font-mono text-[19px] font-black text-white mt-1">{formatDuration(profileStats.totalDurationSeconds)}</div>
        </div>
        <div className="bg-[#141414] p-3 rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_#000000]">
          <div className="text-[11px] text-stone-400 font-bold uppercase tracking-wider font-sans">Avg Workout</div>
          <div className="font-mono text-[19px] font-black text-white mt-1">{formatDuration(profileStats.averageDurationSeconds)}</div>
        </div>
        <div className="bg-[#141414] p-3 rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_#000000]">
          <div className="text-[11px] text-stone-400 font-bold uppercase tracking-wider font-sans">Latest</div>
          <div className="font-mono text-[19px] font-black text-white mt-1">{formatShortDate(profileStats.mostRecentWorkoutDate)}</div>
        </div>
      </div>

      {/* Segmented Control Switcher for Completed Workouts vs Exercise Library */}
      <div className="sticky top-0 z-30 -mx-5  px-5 py-2">
        <div className="bg-[#121011] p-1.5 rounded-xl border border-[#2d2729] grid grid-cols-2 gap-1 font-sans shadow-[0_8px_18px_rgba(0,0,0,0.28)]">
          <button
            ...
            className={`py-2 px-3 rounded-lg text-[13px] font-black uppercase tracking-widest text-center flex items-center justify-center gap-2 transition duration-200 cursor-pointer ${
        ...
        {activeSubTab === "workouts" && (
          <div className="space-y-4">
                ...
                >
                  <div className="flex items-center justify-between font-sans">
                     <div className="font-sans">
                      <h3 className="text-[15px] font-black text-[#f7f5f4]">{log.title}</h3>
                      <p className="text-[11.5px] text-stone-500 font-mono font-bold mt-0.5 tracking-wide uppercase">
                        ✓ Logged {getFriendlyTime(log.end_time)}
                      </p>
                    </div>
                    <span className="text-[11.5px] bg-[#6f6d6c]/10 text-stone-300 border border-[#6f6d6c]/15 px-2 py-0.5 rounded font-mono font-black uppercase">
                      Alex
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 py-2 border-t border-b border-stone-900/65">
                    <div>
                      <span className="text-[11px] uppercase font-black tracking-widest text-stone-550 block">Duration</span>
                      <span className="font-mono text-[15px] font-black text-stone-105">
                        {formatDuration(log.duration_seconds)}
                      </span>
                    </div>
                    <div>
                      <span className="text-[11px] uppercase font-black tracking-widest text-stone-550 block">Volume</span>
                      <span className="font-mono text-[15px] font-black text-stone-105">
                        {Number(log.total_volume).toLocaleString()} <span className="font-sans text-[11px] text-stone-450 uppercase font-bold">kg</span>
                      </span>
                    </div>
                    <div>
                      <span className="text-[11px] uppercase font-black tracking-widest text-stone-550 block">Sets</span>
                      <span className="font-mono text-[15px] font-black text-stone-105 flex items-center gap-1">
                  ...
                  {log.notes && (
                    <p className="text-[13px] text-stone-400 italic bg-stone-900/10 px-3 py-2 rounded-xl border border-white/5">
                      "{log.notes}"
                    </p>
                  )}
                  ...
                  <div className="space-y-3">
                        ...
                        <div key={le.id} className="flex items-center gap-3">
                          ...
                          <div className="text-left flex-1 min-w-0">
                            <p className="text-[13.5px] font-bold text-stone-250 truncate">
                              <span className="font-mono text-stone-450 text-[12.5px] font-black tracking-tight mr-1.5">
                                {displaySetsCount} sets
                              </span>
                              {ex?.name}
                            </p>
                            <span className="text-[11px] bg-[#1c181a] border border-[#2d2729] text-stone-500 px-1.5 py-0.5 rounded font-mono font-bold uppercase tracking-wider mt-0.5 inline-block">
                              {ex?.body_part} target
                            </span>
                          </div>
                        </div>
                    ...
                    {hasMore && (
                      <button
                        ...
                        className="text-[12px] font-black uppercase text-stone-450 hover:text-stone-300 transition tracking-widest flex items-center gap-1 pt-1 cursor-pointer"
                      >
            ...
            {currentUserLogs.length === 0 && (
              <div className="p-12 text-center text-stone-500 bg-[#191919] rounded-2xl border border-dashed border-[#6f6d6c]/25 mt-2 font-sans">
                <Dumbbell className="w-8 h-8 mx-auto mb-2 text-[#6f6d6c]" />
                <p className="text-[15px] font-bold text-stone-400">No workout logs found.</p>
                <p className="text-[12.5px] mt-1 text-[#6f6d6c] font-mono">Unlock by finishing a template workout inside the "Workout" tab!</p>
              </div>
            )}
          </div>
        )}
