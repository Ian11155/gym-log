    ...
    <div id="supabase-code-hub" className="bg-[#121111] border border-white/5 rounded-2xl overflow-hidden shadow-3d-md">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <h3 className="font-bold text-stone-100 flex items-center gap-2">
              ...
              <span className="text-[13px] bg-emerald-500/10 text-emerald-400 font-bold px-2.5 py-0.5 rounded-full border border-emerald-500/20">
                PostgreSQL
              </span>
            </h3>
            <p className="text-[15px] text-stone-400">Planned database migration and TypeScript model reference</p>
          </div>
        </div>
        <button
          onClick={() => handleCopy(codeToShow)}
          className="flex items-center gap-2 px-3.5 py-2 bg-[#1c1b1b] hover:bg-[#252424] active:scale-95 text-[13px] font-black uppercase tracking-widest text-stone-200 rounded-xl transition-all border border-white/5 cursor-pointer"
        >
          {copied ? (
            <>
      ...
      <div className="bg-[#0c0c0d]/80 px-4 py-2 border-b border-white/5 flex gap-2">
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-[13px] font-black uppercase tracking-widest flex items-center gap-2 transition cursor-pointer ${
            activeSubTab === "sql"
      ...
      {/* Code Container */}
      <div className="p-4 bg-[#080809] overflow-x-auto max-h-[380px] text-[15px] font-mono text-stone-300 leading-relaxed scrollbar-thin text-left border-b border-white/5">
        <pre className="whitespace-pre">{codeToShow}</pre>
      </div>

      {/* Helpful Instructions footer */}
      <div className="bg-[#121111]/90 p-5 text-[15px] text-stone-400 text-left">
        <h4 className="font-bold text-stone-300 mb-2.5 flex items-center gap-1.5 uppercase tracking-wide text-[14px]">
